...
const ContactForm = () => {
  ...
  const options1 = [
    {
      label: "Stupire amici e parenti con foto indimenticabili",
      value: "Stupire amici e parenti con foto indimenticabili",
    },
    {
      label: "Condivido i valori di semplicità e naturalezza",
      value: "Condivido i valori di semplicità e naturalezza",
    },
    {
      label: "Vivere un'esperienza unica da poter ricordare attraverso le foto",
      value: "Vivere un'esperienza unica da poter ricordare attraverso le foto",
    },
  ...
  const options2 = [
    {
      label: "Sì, e sto cercando questo stile",
      value: "Sì, e sto cercando questo stile",
    },
    {
      label: "Ne ho sentito parlare ma vorrei più info",
      value: "Ne ho sentito parlare ma vorrei più info",
    },
  ...
  const validateForm = () => {
    ...
    if (!photoServicePreference) {
      valid = false;
      formErrors.photoServicePreference =
        "Seleziona un'opzione per il servizio fotografico";
    ...
    if (
      ...
      formErrors.characteristics =
        "La prima e la seconda caratteristica devono essere diverse";
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      const formDataToSend = {
        ...formData,
        photoServicePreference:
          photoServicePreference === "Altro"
            ? otherPreference1
            : photoServicePreference,
        reportageKnowledge:
          reportageKnowledge === "Altro"
            ? otherPreference2
            : reportageKnowledge,
        albumPreference,
        firstCharacteristic,
        ...
        wedding_date: formData.wedding_date
          ? formData.wedding_date.toLocaleDateString("it-IT", {
              day: "2-digit",
              month: "2-digit",
              year: "numeric",
            })
        ...
        {isSubmitted ? (
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label
                htmlFor="name"
                className="block text-[#102e5e] font-bold mb-2"
              >
                Nome
              </label>
              <input
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-2 border ${
                    errors.cellulare ? "border-red-500" : "border-gray-300"
                  } rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`}
                />
                {errors.cellulare && (
                  <p className="text-red-500 text-sm">{errors.cellulare}</p>
                )}
              </div>
              <div className="mb-4">
                <label
                ...
                />
                {errors.wedding_date && (
                  <p className="text-red-500 text-sm">{errors.wedding_date}</p>
                )}{" "}
                {/* Mostra errore */}
              </div>
            </div>
            ...
            <RadioGroup
              ...
              title={
                "Ora pensa alla tua personalità e scegli la SECONDA caratteristica che più la definisce."
              }
              options={optionsCharacteristics}
              selectedOption={secondCharacteristic}
              onChange={(e) => setSecondCharacteristic(e.target.value)}
            />
            {errors.secondCharacteristic && (
              <p className="text-red-500 text-sm mb-4">
                {errors.secondCharacteristic}
              </p>
            )}
